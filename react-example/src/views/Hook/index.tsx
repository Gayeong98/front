...
export default function Hook() {
    ...
    const onPlusHandler = () => {
        //^ 상태를 set메서드로 변경시키더라도
        //^ 바로 상태가 변경되는 것이 아니라
        //^ 해당 호출 혹은 함수가 종료되고 리랜더링된 후 변경됨
        setNum(num + 1);
        //^ 그렇기 때문에 아래와 같이 변경 후에 변경한 값으로 작업하려고 해도
        //^ 원래 저장되어 있는 값으로 작업이 진행됨
        alert(num);
        //! 이런 문제를 해결하는 방법
        //^ 1. 아래의 useEffect 처럼 사용
    ...
    //^ useEffect에서 scope한 state를 해당 useEffect 내에서
    //^ 변경하는 작업을 진행하면 무한 실행
    useEffect(() => {
        console.log('이게 뭐야!!!!!');
        console.log('이건멍멍이~~');
